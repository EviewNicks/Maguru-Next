...
describe('Module API Integration', () => {
  ...
  it('should fetch and display modules', async () => {
    ...
    expect(screen.getByTestId('module-1')).toHaveTextContent(
      'Modul Test 1 - ACTIVE'
    )
    expect(screen.getByTestId('module-2')).toHaveTextContent(
      'Modul Test 2 - DRAFT'
    )
  })
  ...
  it('should handle error when fetching modules', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Error: Error fetching modules')
      ).toBeInTheDocument()
    })
  })
  ...
  it('should create module with optimistic update', async () => {
    ...
    // Wait for success toast instead of success element
    await waitFor(() => {
  ...
  it('should fetch module by id', async () => {
    ...
    await waitFor(() => {
      const moduleDetailElement = screen.getByTestId(
        `module-detail-${mockModules.data[0].id}`
      )
