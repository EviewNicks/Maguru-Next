...
const ModulePage: React.FC<ModulePageProps> = ({
  moduleId,
  quickViewMode: initialQuickViewMode = false,
}) => {
  ...
  const [userInteractions, setUserInteractions] = useState<Set<string>>(
    new Set()
  )
  ...
  const handleNavigateToPage = useCallback(
    (pageNumber: number, force: boolean = false) => {
  ...
  useEffect(() => {
    if (currentPageData?.isLastPage) {
      ...
      localStorage.setItem(
        `module_last_page_${moduleId}`,
        JSON.stringify(moduleCompletion)
      )
    }
  }, [
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <h1 className="text-3xl font-bold mb-2">
        {sanitizeHtml(currentModule.title)}
      </h1>
      <p className="text-muted-foreground mb-6">
        {sanitizeHtml(currentModule.description)}
      </p>
      ...
      {currentModule.totalPages > 3 && (
        <Card className="mt-8 p-4">
          <div className="flex flex-wrap gap-2 justify-center">
            {Array.from({ length: currentModule.totalPages }).map(
              (_, index) => {
